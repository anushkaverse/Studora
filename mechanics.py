...
def extract_transcript_details(youtube_url):
    try:
        ...
        # Retry-friendly and language-specific
        transcript = YouTubeTranscriptApi.get_transcript(video_id, languages=['en'])
        transcript_text = " ".join([i['text'] for i in transcript])
        return transcript_text

    except (TranscriptsDisabled, NoTranscriptFound) as e:
        print(f"[Transcript Missing] {e}")
        return None
...
def generate_with_gemini(prompt, content):
    try:
        if not content or len(content.strip()) < 50:
            return "Sorry, the transcript seems too short or missing. Try a different video."

        model = genai.GenerativeModel("models/gemini-1.5-flash")

        # Gemini works best with structured inputs
