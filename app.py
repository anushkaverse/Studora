...
# streamlit app
st.set_page_config(page_title="STUDORA", layout="wide")
st.title("🧾 STUDORA :  Next Gen Prepration Buddy")
...
video_id = mechanics.get_video_id(youtube_link)
...
if youtube_link and video_id:
    ...
    if st.button("Generate Study Material"):
        with st.spinner("Extracting transcript and analyzing..."):
            transcript_text = mechanics.extract_transcript_details(youtube_link)

        if transcript_text:
            with st.spinner("Generating Summary..."):
                st.session_state.summary = mechanics.generate_summary(transcript_text, difficulty)
            with st.spinner("Generating Flashcards..."):
                st.session_state.flashcards = mechanics.generate_flashcards(transcript_text)
            with st.spinner("Generating Revision Plan..."):
                st.session_state.revision_plan = mechanics.generate_revision_plan(transcript_text)
            with st.spinner("Finding Extra Resources..."):
                st.session_state.resources = mechanics.generate_resources(transcript_text)
...
with st.sidebar.form("feedback_form"):
    ...
    if submitted:
        with open("feedback.txt", "a", encoding="utf-8") as f:
            f.write(f"Name: {name}\nEmail: {email}\nMessage: {message}\n{'-'*40}\n")
        st.sidebar.success("Thank you for your feedback! Studora at your service!!")